...
async function getOneEvent(eventId) {
  ...
  const { rows } = await pool.query(`
    SELECT
      ...
      l.maximum_vendor_count AS location_maximum_vendor_count,
      l.room_count AS location_room_count,
...
router.get('/:eventId', async ctx => {
  ...
  if (event === null) {
    ...
    return ctx.body = {
      ...
      message: `Could not find the event with identifier ${eventId}`,
    };
  }
...
router.put('/:eventId/:accountId', async ctx => {
  let {
    name,
    from,
    to,
